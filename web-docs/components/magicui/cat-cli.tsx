...
const catFrames = {
  ...
  success: [{ head: "╭˄───────˄╮", face: "│  ^ܫ^    │", chin: "─────────" }],
  idle: [{ head: "╭˄───────˄╮", face: "│  ≽ܫ≼    │", chin: "─────────" }]
}
...
const cliSequence: CLIPhase[] = [
  ...
  {
    text: "Generating TypeScript interfaces...",
    state: "loading",
    duration: 1500,
    line2: "Adding data models and API services"
  },
  {
    text: "Adding Zod validation schemas...",
    state: "loading",
    duration: 1500,
    line2: "Your data safety is our priority 🛡️"
  },
...
const renderHelpContent = (className?: string) => {
  const helpLines = ["Meow! I'm here to help! Here's the", "documentation:                    "]
  ...
  return (
    <div className={cn("text-sm leading-tight font-mono whitespace-pre text-left pt-2", className)}>
      ...
      <div className="mt-4" dangerouslySetInnerHTML={{ __html: content.replace(/\n\s\s/g, "<br/>") }} />
    </div>
  )
}
...
export const CatCLI = ({ mode, className }: CatCLIProps) => {
  ...
  const bubbleLines = currentPhase.line2 ? [currentPhase.text, currentPhase.line2] : [currentPhase.text]
...
└ <span class="text-lime-400">✨ All done!</span> Import from: "frontend\\src\\generated"
    ...
    return <div className="mt-6 text-gray-400" dangerouslySetInnerHTML={{ __html: fileStructure }} />
  }, [currentPhase.state])
  ...
  return (
    <div className={cn("text-sm leading-tight font-mono whitespace-pre text-left", className)}>
      ...
      <div className="text-lime-400 font-bold ml-1">
        ...
        <div className="text-lime-400">╰{currentFrame.chin}╯</div>
      </div>
